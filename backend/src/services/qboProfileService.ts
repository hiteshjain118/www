...
export interface QBOProfileData {
  cbId: bigint;
  ownerId: bigint;
  realmId?: string;
  accessToken?: string;
...
export class QBOProfileService {
   ...
   */
  static async upsertProductionProfile(data: QBOProfileData) {
    return await prisma.qboProfile.upsert({
   ...
   */
  static async upsertSandboxProfile(data: QBOProfileData) {
    return await prisma.qboProfileSandbox.upsert({
      where: { cbId: data.cbId },
      update: {
        ownerId: data.ownerId,
        realmId: data.realmId,
   ...
   */
  static async upsertProfile(data: QBOProfileData, isProduction: boolean = false) {
    if (isProduction) {
   ...
   */
  static async getValidAccessToken(cbId: bigint, isProduction: boolean = false) {
    const profile = await this.getProfile(cbId, isProduction);
